...
void AutonomyNode::create_behavior_tree()
{
  ...
  // Register StopRobot node
  BT::NodeBuilder stop_robot_builder =
      [=](const std::string &name, const BT::NodeConfiguration &config) 
  {
      return std::make_unique<StopRobot>(name, config);
  };
  factory.registerBuilder<StopRobot>("StopRobot", stop_robot_builder);
...
void AutonomyNode::update_behavior_tree()
{
  ...
  else if (tree_status == BT::NodeStatus::SUCCESS)
  {
    ...
    // Stop spinning and shut down the node after finishing navigation
    rclcpp::shutdown();
    return;
  }
